...
# Cấu hình Secret Key bảo mật cho Session
...
def ask_gemini(prompt_text):
    """
    Hàm gọi Gemini AI chính thức qua SDK google-genai.
    Sử dụng cơ chế xoay vòng Key dự phòng.
    """
    if not GEMINI_KEYS:
...
def get_ns_headers(nation_name, password=None):
    """
    Tạo Header chuẩn tuyệt đối theo đúng quy định NationStates API:
    - User-Agent bắt buộc đúng định dạng.
    - X-Password bắt buộc khi truy cập Private Shards / Commands.
    """
...
def login():
    """
    Trang đăng nhập thông tin quốc gia và lưu vào Session.
    """
...
def logout():
    """
    Đăng xuất và xóa thông tin phiên làm việc.
    """
    session.clear()
...
def index():
    ...
    # Header chứa X-Password để không dính lỗi HTTP 403
...
def analyze_issue():
    """
    Nhận yêu cầu phân tích Issue và gửi cho Gemini AI.
    """
...
def respond_issue():
    """
    Gửi lựa chọn giải quyết Issue lên NationStates API (c=issue).
    """
    if 'nation' not in session or 'password' not in session:
        return jsonify({'status': 'error', 'message': 'Chưa đăng nhập quốc gia!'}), 401
