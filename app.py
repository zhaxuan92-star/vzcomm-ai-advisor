...
def respond_issue():
    """Endpoint gửi quyết định Issue lên NationStates API chính xác"""
    issue_id = request.form.get('issue_id')
    option_id = request.form.get('option_id')
    
    if not issue_id or option_id is None:
        return jsonify({'status': 'error', 'message': 'Thiếu dữ liệu issue_id hoặc option_id'}), 400

    url = "https://www.nationstates.net/cgi-bin/api.cgi"
    
    # Chuẩn hóa dữ liệu gửi theo đúng chuẩn NationStates API Spec
    payload = {
        'nation': NATION_NAME,
        'c': 'issue',
        'issue': str(issue_id),
        'action': 'respond',
        'option': str(option_id)  # NationStates nhận tham số 'option' cho ID phương án được chọn
    }
    
    headers = HEADERS.copy()
    if NATION_PASSWORD:
        headers['X-Password'] = NATION_PASSWORD
        # Nếu bạn dùng Autologin PIN từ Cookie thay vì Password thật, hãy bỏ comment dòng dưới:
        # headers['X-Autologin'] = NATION_PASSWORD
    
    print(f"\n[DEBUG] 📤 Đang gửi quyết định lên NationStates...")
    print(f"[DEBUG] Payload: {payload}")
    
    try:
        res = requests.post(url, data=payload, headers=headers, timeout=10)
        print(f"[DEBUG] 📥 Kết quả từ Server: HTTP {res.status_code}")
        print(f"[DEBUG] Response Text: {res.text}")
        
        # Lỗi thường gặp: Nếu trả về XML có chứa thẻ <ERROR>
        if res.status_code == 200 and "<ERROR>" not in res.text:
            ...
            # Trích xuất thông báo lỗi từ XML trả về nếu có
            error_msg = res.text
            if "<ERROR>" in res.text:
                ...
                try:
                    ...
                    pass
            return jsonify({'status': 'error', 'message': f'NationStates từ chối: {error_msg}'}), 400

    except Exception as e:
        print(f"[DEBUG ERROR] Lỗi gửi request: {e}")
